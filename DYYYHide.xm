...
// 定义配置键名（避免硬编码）
static NSString *const kFollowCountKey = @"DYYYhz";
static NSString *const kFriendCountKey = @"DYYYhg";
static NSString *const kFansCountKey = @"DYYYfs";
...
- (void)layoutSubviews {
    ...
    NSDictionary *replaceRules = @{
        @"follow" : @{
            @"originalValue" : @(60000),          // 原数值（单位：万）
            @"userDefaultsKey" : kFollowCountKey
        },
        @"friend" : @{
            @"originalValue" : @(1),
            @"userDefaultsKey" : kFriendCountKey
        },
        @"fans" : @{
            @"originalValue" : @(183),
            @"userDefaultsKey" : kFansCountKey
        }
    };
    ...
    for (UIView *subview in self.subviews) {
        ...
        // 遍历所有替换规则
        [replaceRules enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSDictionary *rule, BOOL *stop) {
            // 转换原文本为数值（支持万/千单位）
            NSNumber *originalNumber = [self parseNumberFromString:originalText];
            
            if ([originalNumber isEqualToNumber:rule[@"originalValue"]]) {
                NSString *customValue = [[NSUserDefaults standardUserDefaults] objectForKey:rule[@"userDefaultsKey"]];
                
                if (customValue.length > 0) {
                    // 保留原格式的智能替换（单位/分隔符）
                    label.text = [self formattedString:customValue basedOnOriginal:originalText];
                    
                    // 避免触发循环布局
                    [self markLabelForStyleUpdate:label];
                }
            }
        }];
    }
...
- (NSNumber *)parseNumberFromString:(NSString *)string {
    ...
    // 处理中文单位（万/千）
    if ([string containsString:@"万"]) {
        formatter.multiplier = @(10000);
    } else if ([string containsString:@"千"]) {
        formatter.multiplier = @(1000);
    }
    
    return [formatter numberFromString:[string stringByReplacingOccurrencesOfString:@"." withString:@""]];
}

// 保留原格式的文本生成
- (NSString *)formattedString:(NSString *)value basedOnOriginal:(NSString *)original {
    // 检测原文本是否包含单位
    NSCharacterSet *unitSet = [NSCharacterSet characterSetWithCharactersInString:@"万千"];
    NSRange unitRange = [original rangeOfCharacterFromSet:unitSet];
    
    if (unitRange.location != NSNotFound) {
        NSString *unit = [original substringWithRange:unitRange];
...
// 安全更新样式（避免循环布局）
- (void)markLabelForStyleUpdate:(UILabel *)label {
