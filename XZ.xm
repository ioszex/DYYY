...
- (NSArray *)sectionDataArray {
    ...
    if (self.traceEnterFrom && !sectionExists) {
        AWESettingItemModel *newItem = [self createSettingItemWithIdentifier:DYYY title:DYYY detail:tweakVersion type:0 imageName:@"noticesettting_like" cellType:26 colorStyle:2 isEnable:YES svgIcon:NO];
...
- (NSArray *)dataArray {
    ...
    for (NSUInteger i = 0; i < customButtons.count; i++) {
        ...
        bool allImages = [customButtons[i] isEqualToString:@"下载全部图片"];
